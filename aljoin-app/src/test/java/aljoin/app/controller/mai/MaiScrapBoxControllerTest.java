...
public class MaiScrapBoxControllerTest extends BaseTest {
	...
	public void testAdd() throws Exception {
		String uri = "http://localhost:8080/app/mai/maiScrapBox/add";
		MockHttpServletRequestBuilder mhsrb = MockMvcRequestBuilders.post(uri);
		mhsrb.param("createUserId", "866131328341528576");
		mhsrb.param("receiveUserIds", "903158226865868800");
		mhsrb.param("receiveUserNames", "sadmin");
		mhsrb.param("receiveFullNames", "超级管理员");
		mhsrb.param("sendUserId", "866131328341528576");
		mhsrb.param("sendUserName", "laijy");
		mhsrb.param("sendFullName", "赖建洋");
		mhsrb.param("subjectText", "测试草稿");
		mhsrb.param("mailSize", "100");
		mhsrb.param("orgnlType", "1");
		mhsrb.param("orgnlId", "918760867402444800");
		...
		ipList.add(new InterfaceParam("orgnlId", "数字", "不可为空", "源ID，针对orgnl_type,对应相应的表：mai_receive_box,mai_send_box",
				"不加密"));
	...
	public void testList() throws Exception {
		String uri = "http://localhost:8080/app/mai/maiScrapBox/list";
		MockHttpServletRequestBuilder mhsrb = MockMvcRequestBuilders.post(uri);
	...
	public void testDelete() throws Exception {
		String uri = "http://localhost:8080/app/mai/maiScrapBox/delete";
		MockHttpServletRequestBuilder mhsrb = MockMvcRequestBuilders.post(uri);
		// mhsrb.param("id", "918015886953910272");
		MvcResult mvcResult = mvc.perform(mhsrb.accept(MediaType.APPLICATION_JSON)).andReturn();
		int status = mvcResult.getResponse().getStatus();
		String content = mvcResult.getResponse().getContentAsString();
		InterfaceObject io = new InterfaceObject();
		...
		/* 设置接口参数 */
		List<InterfaceParam> ipList = new ArrayList<InterfaceParam>();
		ipList.add(new InterfaceParam("id", "数字", "不可为空", "主键ID(选中多个废件则要传多个)", "不加密"));
		io.setParamList(ipList);
		/* 设置接口结果集以及加密字段 */
		io.setRetObj(JsonUtil.str2json(content));
