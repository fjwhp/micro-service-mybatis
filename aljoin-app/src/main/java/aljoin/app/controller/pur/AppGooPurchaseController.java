package aljoin.app.controller.pur;
...
@Controller
@RequestMapping(value = "/app/goo/gooPurchase", method = RequestMethod.POST)
@Api(value = "办公用品申购Controller", description = "办公用品申购相关接口")
public class AppGooPurchaseController extends BaseController {
 private final static Logger logger = LoggerFactory.getLogger(AppGooPurchaseController.class);
  ...
  public RetMsg completeTask(HttpServletRequest request, HttpServletResponse response,
      ...
      try {
          ...
          retMsg = gooPurchaseService.completeAppTask(variables, taskId, bizId, userId, message, autUser);
