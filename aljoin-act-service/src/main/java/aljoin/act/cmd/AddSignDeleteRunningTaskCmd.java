 ...
 * @date：2018年03月19日
 */
public class AddSignDeleteRunningTaskCmd implements Command<Void> {
    private TaskEntity currentTaskEntity;

    private final static Logger logger = LoggerFactory.getLogger(AddSignDeleteRunningTaskCmd.class);

    public AddSignDeleteRunningTaskCmd(TaskEntity currentTaskEntity) {
    ...
    public Void execute(CommandContext commandContext) {
        ...
        if (currentTaskEntity != null) {
            ...
            Context.getCommandContext().getTaskEntityManager().deleteTask(currentTaskEntity,
                WebConstant.ADDSIGN_REASON, false);
