...
public class CustomProcessDiagramGenerator implements ProcessDiagramGenerator {
    ...
    // 执行过的节点和当前节点样式不一样
    public InputStream generateDiagram2(BpmnModel bpmnModel, String imageType, List<String> highLightedActivities,
        List<String> highLightedFlows, String activityFontName, String labelFontName, ClassLoader customClassLoader,
        double scaleFactor, List<String> currentActivities) {

        return generateProcessDiagram2(bpmnModel, imageType, highLightedActivities, highLightedFlows, activityFontName,
            labelFontName, customClassLoader, scaleFactor, currentActivities).generateImage(imageType);
    }
    ...
    protected CustomProcessDiagramCanvas generateProcessDiagram2(BpmnModel bpmnModel, String imageType,
        List<String> highLightedActivities, List<String> highLightedFlows, String activityFontName,
        String labelFontName, ClassLoader customClassLoader, double scaleFactor, List<String> currentActivities) {
        ...
        // Draw activities and their sequence-flows
        for (FlowNode flowNode : bpmnModel.getProcesses().get(0).findFlowElementsOfType(FlowNode.class)) {
            drawActivity2(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows,
                scaleFactor, currentActivities);
        }
    ...
    protected void drawActivity(CustomProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode,
        ...
        if (drawInstruction != null) {
            ...
            if (highLightedActivities.contains(flowNode.getId())) {
                ...
                if (flowNode.getClass().toString().endsWith(".StartEvent")) {
                    defaultRadius = 50;
                }
                drawHighLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()), defaultRadius);
            }
    ...
    protected void drawActivity2(CustomProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
        ...
        if (drawInstruction != null) {
            ...
            if (currentActivities.contains(flowNode.getId())) {
                ...
                drawHighLight2(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()), defaultRadius);
    ...
    private static void drawHighLight(CustomProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo,
        ...
        processDiagramCanvas.drawHighLight((int)graphicInfo.getX(), (int)graphicInfo.getY(),
            (int)graphicInfo.getWidth(), (int)graphicInfo.getHeight(), radius);

    }

    private static void drawHighLight2(CustomProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo,
        int radius) {
        processDiagramCanvas.drawHighLight2((int)graphicInfo.getX(), (int)graphicInfo.getY(),
            (int)graphicInfo.getWidth(), (int)graphicInfo.getHeight(), radius);
