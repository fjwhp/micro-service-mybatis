	...
	function getHtml(data) {
		...
		for(var i = 0; i < data.length; i++) {
			if(data[i].spread) {
				...
				//回填选中状态
				if(currentLeftMenuId__ == data[i].id){
			...
			if(data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
				ulHtml += '<a id="'+data[i].id+'" href="javascript:;">';
				...
				for(var j = 0; j < data[i].children.length; j++) {
					...
					ulHtml += '<a id="'+data[i].children[j].id+'" href="javascript:;" data-url="' + data[i].children[j].href + '">';
				...
				ulHtml += '<a id="'+data[i].id+'" href="javascript:;" ' + dataUrl + '>';
				if(data[i].icon !== undefined && data[i].icon !== '' && data[i].icon !== null) {
					if(data[i].icon.indexOf('fa-') !== -1) {
						ulHtml += '<i class="fa ' + data[i].icon + '" aria-hidden="true" data-icon="' + data[i].icon + '"></i>';
					} else {
