...
layui.define(['element', 'common'], function (exports) {
	 ...
	 * @param {String} 标题
	 */
    Tab.prototype.exists = function (id_) {
        var that = ELEM.titleBox === undefined ? this.init() : this,
        tabIndex = -1;
        ELEM.titleBox.find('i.layui-tab-close').each(function (i, e) {
            if ($(this).attr("id") === id_) {
                tabIndex = i;
            };
        });
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            globalTabIdIndex++;
            var indexPageId="";
            if(data.href=="/tempMenu/indexPage"){
            	indexPageId = "indexPageIframe";
            	tool.post("/aut/autDataStatistics/checkMsg",{_csrf:$("#_csrf").val()},function(data){
            		var i = Number(data.object);
                	//$("#mywidth").children(".onNum").text(i); 
            	}, true);           	
            }else{
            	indexPageId = "notIndexPageIframe";
            }
            var content = '<iframe src="' + data.href + '" data-id="' + globalTabIdIndex + '" id="'+indexPageId+'"></iframe>';
            var title = '';
            //一下注释去掉tab前面的图标
            ...
            if (_config.closed) {
                title += '<i class="layui-icon layui-unselect layui-tab-close" id="'+data.id+'" data-id="' + globalTabIdIndex + '">&#x1006;</i>';
            }
            //添加tab
