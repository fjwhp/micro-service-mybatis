...
public class ioaCreateWorkTest extends BaseTest {
  ...
  public void testGetCategoryTree() throws Exception {
    String uri = "http://localhost:8080/ioa/ioaCreateWork/getCategoryTree";
   ...
   */
  @Test
  public void testGetBpmn() throws Exception {
    String uri = "http://localhost:8080/ioa/ioaCreateWork/getBpmn";
    MockHttpServletRequestBuilder mhsrb = MockMvcRequestBuilders.post(uri);
    // mhsrb.param("merchantId", "1098e1ea3bc346c9bc9a284898b8f462");
    mhsrb.param("categoryId", "903419261669433344");
    ...
    io.setInterfaceVersion("v1");
    /* 设置接口名称 */
    io.setInterfaceName("新建工作-根据流程分类以及用户权限获取流程");
    /* 设置接口地址 */
    io.setInterfaceAddress(uri);
    /* 设置接口参数 */
    List<InterfaceParam> ipList = new ArrayList<InterfaceParam>();
    ipList.add(new InterfaceParam("categoryId", "数字", "不可为空", "流程分类id","不加密"));
    io.setParamList(ipList);
    /* 设置接口结果集以及加密字段 */
    io.setRetObj(JsonUtil.str2json(content));
    //io.setRetEncryptProps("userName3,userName3");
