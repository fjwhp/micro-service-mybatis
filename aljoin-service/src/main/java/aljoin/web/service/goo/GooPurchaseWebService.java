...
@Component
public class GooPurchaseWebService {
  ...
  @Transactional
  public RetMsg add(GooPurchaseVO obj, AutUser autuser) throws Exception {
    ...
    if(obj != null) {
      if(null == obj.getProcessName()) {
        obj.setProcessName(autuser.getFullName() + "办公用品申购");
      }
      
      List<GooPurchase> purchaseList = obj.getGooInfoList();
      Where<ActAljoinFixedConfig> configWhere = new Where<ActAljoinFixedConfig>();
      configWhere.eq("process_code", WebConstant.FIXED_FORM_PROCESS_GOO_PURCHASE);
      ...
      if (null != config && null != config.getProcessId()) {
        ...
        retMsg.setCode(1);
        retMsg.setMessage("没有找到办公用品申购流程，请先新增流程");
        return retMsg;
    }
      if (null != purchaseList && !purchaseList.isEmpty()) {
        String ids = "";
        for (GooPurchase gooInfo : purchaseList) {
          if(null != bpmn) {

            Long id = IdWorker.getId();
            if(null == gooInfo.getListCode() || StringUtils.isEmpty(gooInfo.getListCode())) {
              gooInfo.setListCode(id.toString());
            }
            gooInfo.setAuditStatus(2);
            gooInfo.setProcessId(bpmn.getProcessId());
            gooPurchaseService.insert(gooInfo);
            if (null != gooInfo && null != gooInfo.getId()) {
              ids += gooInfo.getId() + ";";
            }
          }
        }
      /*GooPurchase gooPurchase = new GooPurchase();
      obj.setId(null);
      BeanUtils.copyProperties(obj, gooPurchase);
      gooPurchaseService.insert(gooPurchase);*/
      if (null != bpmn) {
        // 启动流程
        if (null != obj.getCreateUserId()) {
            Map<String, String> param = new HashMap<String, String>();
            param.put("bizType", "pur"); 
            param.put("bizId", ids);
              ...
              for (GooPurchase gooInfo : purchaseList) {
                if(null != bpmn) {
                  if (StringUtils.isNotEmpty(processInstanceId)) {
                    gooInfo.setGooPurchaseProcInstId(processInstanceId);
                  } 
                  gooPurchaseService.updateById(gooInfo);
                }
            }
            ...
            taskService.claim(task.getId(), String.valueOf(obj.getCreateUserId()));
            String userFullName = "";
  ...
  public RetMsg completeTask(Map<String, Object> variables, String taskId, String bizId,
    ...
    Date date = new Date();
    GooPurchase gooPurchase = gooPurchaseService.selectById(Long.parseLong(bizId));
    if (StringUtils.isNotEmpty(bizId)) {
      ...
      List<GooPurchase> gooPurchaseList = gooPurchaseService.selectList(where);
      ...
      if (null != gooPurchaseList && !gooPurchaseList.isEmpty()) {
        List<GooPurchase> purchaseList = new ArrayList<GooPurchase>();
          ...
          if (null == pi) {
              ...
              gooPurchaseService.updateById(gooPurchase);
              map.put("isEnd", "1");
          }
      }
    }
