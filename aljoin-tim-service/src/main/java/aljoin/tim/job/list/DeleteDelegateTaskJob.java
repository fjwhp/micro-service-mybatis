 ...
 */
public class DeleteDelegateTaskJob implements BaseJob {

    private final static Logger logger = LoggerFactory.getLogger(DeleteDelegateTaskJob.class);

    @Override
    public void execute(Map<String, Object> map) {
        logger.info("开始执行[(结束时间)删除委托任务定时器]");
        try {
            // (结束时间)删除委托
            ...
            actAljoinDelegateService.timerDelegateBiz(actAljoinDelegate, false);

            // (结束时间)删除定时器(此处存在事务不一致的风险)
            ...
            logger.error("开始执行[(结束时间)删除委托任务定时器]", e);
        }
    }
